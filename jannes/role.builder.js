        ...
        if (creep.memory.full) {
            ...
            if (targets) {
                ...
                for (let myRooms = 0, length = Memory.spawns[creep.memory.spawn].random.rooms.length; myRooms < length; myRooms++) {
                    if (Game.rooms[Memory.spawns[creep.memory.spawn].random.rooms.name[myRooms]] && creep.pos.roomName != Game.rooms[Memory.spawns[creep.memory.spawn].random.rooms[myRooms]].name) {
                        var tmpTargets = Game.rooms[Memory.spawns[creep.memory.spawn].random.rooms.name[myRooms]].find(FIND_MY_CONSTRUCTION_SITES);
