    ...
    run: function (h, b, u, h2, b2, u2, atk, harvesters, builders, upgraders, harvesters2, builders2, upgraders2, attackers, scouts, stores, sources, defenders, spawn) {
        ...
        if (didSpawn == false) {
            ...
            if (builders.length < b) {
                ...
                if (!_.filter(Game.creeps, (creep) => creep.memory.role == 'extractor' && creep.memory.spawn == spawn).length && Game.getObjectById(Memory.spawns[spawn].random.extractor).mineralAmount > 0)
