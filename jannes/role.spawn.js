var roleSpawn = {
    ...
    run: function (h, b, u, h2, b2, u2, atk, harvesters, builders, upgraders, harvesters2, builders2, upgraders2, attackers, scouts, stores, sources, defenders, spawn) {
            ...
            if (didSpawn == false) {
                for (let s = 0, length = sources.length; s < length; s++) {
                    ...
                    if (!filterLength.length || (filterLength.length == 1 && filterLength[0].ticksToLive < 80)) {
                        ...
                        if (didSpawn == false) {
                            var newName5 = Game.spawns['Spawn' + parseInt(spawn + 1)].createCreep([WORK, WORK, WORK, WORK, WORK, WORK, WORK, CARRY, MOVE, MOVE, MOVE, MOVE], undefined, { role: 'store', sourceId: sources[s], spawn: spawn });
                            didSpawn = true;
                            break;
                        }
                    }
        ...
        if (didSpawn == false) {
            ...
            else if (Memory.spawns[creep.memory.spawn].random.extractor) {
                if(!_.filter(Game.creeps, (creep) => creep.memory.role == 'extractor' && creep.memory.spawn == spawn).length)
