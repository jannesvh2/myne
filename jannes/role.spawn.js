var roleSpawn = {
    ...
    run: function (h, b, u, h2, b2, u2, atkM, atkR, harvesters, builders, upgraders, harvesters2, builders2, upgraders2, attackersM, attackersR, scouts, stores, sources, defenders, spawn) {
        ...
        if (didSpawn == false) {
            ...
            else if (Memory.spawns[spawn].random.extractor && Memory.spawns[spawn].random.terminal && Game.getObjectById(Memory.spawns[spawn].random.extractor).mineralAmount > 0 && _.sum(Memory.spawns[spawn].random.terminal.store) < Memory.spawns[spawn].random.terminal.storeCapacity) {
